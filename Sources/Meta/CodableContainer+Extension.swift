...
// MARK: - Lossy Value Type Decoding
  ...
  mutating func decode(_ type: Array<Any>.Type) throws -> Array<Any> {
    ...
    while isAtEnd == false {
      ...
      if try decodeNil() {
        ...
        var nestedUnkeyedContainer = try? self.nestedUnkeyedContainer()
        if let nestedArray = try? nestedUnkeyedContainer?.decode(Array<Any>.self) {
  ...
  mutating func encode(_ value: Array<Any>) throws {
    try value.enumerated().forEach({ (index, value) in
      ...
      case let value as Dictionary<String, Any>:
        try encode(value)
      case let value as Array<Any>:
        var nestedUnkeyedContainer = nestedUnkeyedContainer()
